	...
	if(validate!=0){
            ...
            $.ajax({
                ...
                error: ()=>{
                    ...
                    $('#modal-update-item').modal('hide');
						$('#modal-message-box').find('.modal-body').text("SucccessFul Updating Data");
						$('#modal-message-box').modal('show');
                        // action = event.action;
                        show_all_item();
                }
...
// 					$('#fN-view').val(data.accountname);
// 					$('#mN-view').val(data.username);
// 					$('#lN-view').val(data.password);
// 					$('#email-view').val(data.email);
// 					$('#offices-view').val(data.office);
// 					$('#pos-view').val(data.position);
	...
	$.ajax({
			...
			success: function (data) {
				...
				if(data){
					...
					$('#mN-viewed').val(data.username);
					$('#lN-viewed').val(data.password);
